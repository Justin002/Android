...
public class MediaManage implements Observer {
	 ...
	 * 播放歌曲类型
	 */
	private int playListType = SongStatics.playListType;
	/**
	...
	public void initSongInfoData(int mPlayListType) {
		...
		String sid = SongStatics.playInfoID;
...
//		DataUtil.saveValue(context, SongStatics.playListType_KEY,
//				SongStatics.playListType);
	}
	...
	private void preMusic(int playModel) {
		...
		SongStatics.playInfoID = songInfo.getSid();

		new Thread() {

			@Override
			public void run() {
//				DataUtil.saveValue(context, SongStatics.playInfoID_KEY,
//						SongStatics.playInfoID);
			}
...
//						SongStatics.playInfoID);
	...
	public int getPlayIndex() {
		...
		for (int i = 0; i < playlist.size(); i++) {
			...
			if (tempSongInfo.getSid().equals(SongStatics.playInfoID)) {
				return i;
			}
		}
		return index;
	...
	public void update(Observable arg0, Object data) {
		if (data instanceof SongMessage) {
			...
			if (songMessage.getType() == SongMessage.ADDMUSIC
				...
				if (playListType == SongStatics.playListType) {
					//列表的数据发生变化，更新播放列表
					// playListType = SongStatics.playListType;
				...
				nextMusic(SongStatics.playModel);
				...
				preMusic(SongStatics.playModel);
