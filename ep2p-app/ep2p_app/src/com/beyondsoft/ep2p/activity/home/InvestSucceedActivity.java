 ...
 *
 */
public class InvestSucceedActivity extends BaseActivity implements OnClickListener
{
    ...
    private TextView tv_user_code,tv_moder_amount,tv_moder_amount_jl,tv_moder_anticipated_income,tv_moder_deadline,tv_moder_rate;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        mContext = InvestSucceedActivity.this;
        ActivityManager.getInstance().pushActivity(InvestSucceedActivity.this);
        initTitle();
        initView();
        initData();
    }
    ...
    private ExperienceInvestResponse mExperienceInvestResponse = null ;//体验金投资成功的数据bean
    private boolean isResult_finish = false ; //是否投资成功
    private void initData()
    {
        if(getIntent()!=null){
            isShowUI = getIntent().getIntExtra(Constants.EP2P, 0);
            mExperienceInvestResponse = (ExperienceInvestResponse) getIntent().getSerializableExtra("Bean_mExperienceInvestResponse");
            
            isResult_finish = getIntent().getBooleanExtra("result_finish", false);
            Logs.d("isShowUI=："+isShowUI+"\n Bean=:"+mExperienceInvestResponse);
        }
        if(isResult_finish){
        ...
        if(mTzSucceedInfoBean!=null){
            ...
            tv_moder_amount.setText("￥"+ StringUtils.formatMoney(Double.parseDouble(mTzSucceedInfoBean.getTzMoney())));
            if(!TextUtils.isEmpty(mTzSucceedInfoBean.getTzjl())){
                tv_moder_amount_jl.setText("￥"+ StringUtils.formatMoney(Double.parseDouble(mTzSucceedInfoBean.getTzjl())));
            }else{
                tv_moder_amount_jl.setText("");
            }
            if (!TextUtils.isEmpty(mTzSucceedInfoBean.getTzyqsy()))
            {
                tv_moder_anticipated_income.setText("￥" + StringUtils.formatMoney(Double.parseDouble(mTzSucceedInfoBean.getTzyqsy())));
            }
            else
            {
        ...
        if(mExperienceInvestResponse!=null){  //体验金 投资成功的数据Data
            tv_title_isResult_finish.setText("恭喜您，投资成功!");
            tv_user_code.setText(mExperienceInvestResponse.result.borrowCode);
            tv_moder_amount.setText("￥"+ StringUtils.formatMoney(Double.parseDouble(mExperienceInvestResponse.result.amount)));

//          rl_moder_anticipated_income.setVisibility(View.GONE);
            tv_moder_anticipated_income.setText("￥"+ StringUtils.formatMoney(mExperienceInvestResponse.result.interest)); //预期收益
            
            rl_moder_amount_jl.setVisibility(View.GONE);
//          tv_moder_amount_jl.setText(mTzSucceedInfoBean.getTzjl());
            
            tv_moder_deadline.setText(mExperienceInvestResponse.result.borDeadline+"月");
            
            float mNhllProgress = Float.parseFloat(mExperienceInvestResponse.result.borrowRate) * 100;//目标进度
            BigDecimal bd = new BigDecimal(mNhllProgress);
            mNhllProgress = bd.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();//取2位小数[取值条件：四舍五入]
//            tv_moder_rate.setText(mNhllProgress+"%");
            tv_moder_rate.setText(new DecimalFormat("#,##0.00").format(mNhllProgress) + "%");//保留后面两位，不足用0代替
        }
    }
