...
public class RecommendListAdapter<T> extends BaseAdapter
{
    ...
    public RecommendListAdapter(Context ct, List<T> mList)
    {
        super();
        this.mContext = ct;
    ...
    public RecommendListAdapter(Context ct, List<T> mList, int falg,long time)
    {
        super();
        this.mContext = ct;
        this.mInflater = (LayoutInflater) ct
...
//        }
        else if (mViewHolder.tv_join_buttons.getTag() == null || mCFPTypeItem.borStatus.equals("1")||mCFPTypeItem.borStatus.equals(mViewHolder.tv_join_buttons.getTag())) { //显示时间 :  第一个 当标的状态为1（待发布） 并且发布时间不为空 时  显示倒计时
