...
import com.beyondsoft.ep2p.activity.mine.adapter.CreditorRightsTransfDoingAdapter;
 ...
 *
 */
public class CreditorRightsTransfDoingFragment extends BaseFragment {

	private View mParent;

	private ListView lv_content;
	private CreditorRightsTransfDoingAdapter creditorRightsTransfDoingAdapter;
...
//				intent.putExtra(CreditorRightsDetailActivity.FLAG_STATE, CreditorRightsDetailActivity.STATE_DOING);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		mParent = inflater.inflate(R.layout.fragment_creditor_right_doing, null);
		return mParent;
	}
	...
	private void initView() {
		...
		creditorRightsTransfDoingAdapter = new CreditorRightsTransfDoingAdapter(mContext, mHandler);
		lv_content.setAdapter(creditorRightsTransfDoingAdapter);
	}
		...
		case TAG_CREDITOR_RIGHTS_LIST: {
			...
			if (null != creditorRightsListResponse) {
				...
				if (null != creditorRightsListBean) {
					...
					creditorRightsTransfDoingAdapter.setData(mCreditorRightsInfoBeanList);
				}
			}
		}
			break;
	...
	public void onErrorResponse(int tag, String error) {
		...
		switch (tag) {
		case TAG_CREDITOR_RIGHTS_LIST: {
			Toast.makeText(mContext, "2返回参数错误", 3000).show();
		}
			break;
		default:
			break;
