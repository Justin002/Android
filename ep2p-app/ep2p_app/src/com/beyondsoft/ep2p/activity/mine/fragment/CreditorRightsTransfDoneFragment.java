...
import com.beyondsoft.ep2p.activity.mine.adapter.CreditorRightsTransfDoneAdapter;
 ...
 *
 */
public class CreditorRightsTransfDoneFragment extends BaseFragment {

	private View mParent;

	private ListView lv_content;
	private CreditorRightsTransfDoneAdapter creditorRightsTransfDoneAdapter;
...
//				intent.putExtra(CreditorRightsDetailActivity.FLAG_STATE, CreditorRightsDetailActivity.STATE_DONE);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		mParent = inflater.inflate(R.layout.fragment_creditor_right_done, null);
		return mParent;
	}
	...
	private void initView() {
		...
		creditorRightsTransfDoneAdapter = new CreditorRightsTransfDoneAdapter(mContext, mHandler);
		lv_content.setAdapter(creditorRightsTransfDoneAdapter);
	}
		...
		case TAG_CREDITOR_RIGHTS_LIST: {
			...
			if (null != creditorRightsListResponse) {
				...
				if (null != creditorRightsListBean) {
					...
					creditorRightsTransfDoneAdapter.setData(mCreditorRightsInfoBeanList);
				}
			}
		}
			break;
	...
	public void onErrorResponse(int tag, String error) {
		...
		switch (tag) {
		case TAG_CREDITOR_RIGHTS_LIST: {
			Toast.makeText(mContext, "3返回参数错误", 3000).show();
		}
			break;
		default:
			break;
